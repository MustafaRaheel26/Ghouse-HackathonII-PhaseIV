...
const EmptyState = ({
  ...
  const getIcon = () => {
    switch (type) {
      case "all-complete":
        return <Flame className="w-12 h-12 text-primary" />;
      case "no-results":
  ...
  return (
    ...
    >
      <div className="relative mb-8">
        ...
        <div className="absolute -top-2 -right-2 w-8 h-8 rounded-xl bg-orange-500 text-white flex items-center justify-center shadow-lg animate-pulse">
          <Sparkles className="w-4 h-4" />
        </div>
      </div>

      <h2 className="h3 mb-3">{title}</h2>
      <p className="body max-w-sm mb-10 opacity-70">{description}</p>
      ...
      {cta && (
        <Button
          ...
          size="lg"
          onClick={cta.action}
          className="px-8 h-12 rounded-2xl font-bold shadow-xl shadow-orange-500/20"
        >
          {cta.text}
        </Button>
      )}
